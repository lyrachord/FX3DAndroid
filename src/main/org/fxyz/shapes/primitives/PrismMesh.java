...
//            baseVertices[6*div+5]=0f;
            ...
            faces0 = IntStreams.range(0, baseFaces.length/3)
                        .mapToObj(i->IntStreams.of(baseFaces[3*i], baseTexture[3*i],
...
//        points1.forEach(p->System.out.print(" "+p.f));
        ...
        if(level==getLevel()){
            ...
            smoothingGroups=IntStreams.range(0,listFaces.size()).map(i->{
                if(getSectionType()!=TriangleMeshHelper.SectionType.CIRCLE){
                    return 0;
                }
