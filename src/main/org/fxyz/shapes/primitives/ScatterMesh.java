...
//            System.out.println("Single mesh created");
    ...
    private void createDots() {
        if(!joinSegments.get()){
            ...
            Iterables.forEach(scatterData.get(), point3d->{
...
//            TexturedMesh dot1 = new CuboidMesh(height.get(), height.get(), height.get(), level.get(), null);
            ...
            mh.addMesh(mh1,stream(scatterData.get()).skip(1).collect(Collectors.toList()));
            dot.updateMesh(mh);
            meshes.add(dot);
        }
    }
    ...
    public TexturedMesh getMeshFromId(String id){
        return stream(meshes).filter(p->p.getId().equals(id)).findFirst().orElse(meshes.get(0));
    }
