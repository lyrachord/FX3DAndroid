...
//        Function<? extends Integer, ? extends Vector3d> indexToPoint = (Integer i) -> {
//            return points.get(i).clone();
//        };

//        Function<List<Integer>, Polygon> faceListToPolygon
//                = (List<Integer> faceList) -> {
//                    return  Polygon.fromPoints(stream(faceList).map(i->points.get(i).clone()).
//                            collect(Collectors.toList()), properties);
//                };
        return stream(faces).map(faceList-> Polygon.fromPoints(stream(faceList).map(i->points.get(i).clone()).
                collect(Collectors.toList()), properties)).
