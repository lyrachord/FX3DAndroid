        ...
        protected Task<Void> createTask() {
            return new Task<Void>() {
                ...
                protected Void call() throws Exception {
                    ...
                    IntStreams.range(0, getIterations()).forEach(i->{});
                    parallelStream(points).filter(p->{return points.indexOf(p) % (getDivisionsX() - 1) == 0;}).forEach(p -> {
                        p.applyForce(new Point3D(5,-1,1));
                    });
                    for (int i = 0; i < getConstraintAccuracy(); i++) {
                        parallelStream(points).forEach(WeightedPoint::solveConstraints);
                    }
                    parallelStream(points).forEach(p -> {
