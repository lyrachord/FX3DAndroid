...
public class Text3DHelper {
    ...
    public List<Point3D> getOffset(){
        return stream(polis).sorted((p1,p2)->(int)(p1.getOrigen().x-p2.getOrigen().x))
    ...
    private void getPoints(PathElement elem){
        if(elem instanceof MoveTo){
            ...
            IntStreams.rangeClosed(1, POINTS_CURVE).forEach(i->list.add(evalCubicBezier((CubicCurveTo)elem, ini, ((double)i)/POINTS_CURVE)));
        } else if(elem instanceof QuadCurveTo){
            Point3D ini = (list.size()>0?list.get(list.size()-1):p0);
            IntStreams.rangeClosed(1, POINTS_CURVE).forEach(i->list.add(evalQuadBezier((QuadCurveTo)elem, ini, ((double)i)/POINTS_CURVE)));
    ...
    private double getArea(){
        ...
        IntStreams.range(0, list.size()-1)
