...
public class TriangulatedMesh extends TexturedMesh {
    ...
    private TriangleMesh createMesh(int level){
        ...
        if(level==0){
            ...
            List<Integer> duplicates=IntStreams.range(0, pointsExterior.size()).boxed()
            ...
            stream(duplicates).sorted(Collections.reverseOrder()).forEach(i->pointsExterior.remove(i.intValue()));

            List<PolygonPoint> list = stream(pointsExterior).map(p->new PolygonPoint(p.x, p.y)).collect(Collectors.toList());
            ...
            if(bounds.get()!=null){
                ...
                maxX = stream(pointsExterior).mapToDouble(p->p.x).max().getAsDouble();
                maxY = stream(pointsExterior).mapToDouble(p->p.y).max().getAsDouble();
                minX = stream(pointsExterior).mapToDouble(p->p.x).min().getAsDouble();
                minY = stream(pointsExterior).mapToDouble(p->p.y).min().getAsDouble();
            }
            ...
            if(pointsHoles!=null){
                ...
                // holes
                Iterables.forEach(pointsHoles, pHole->{
                    // hole
                    List<PolygonPoint> hole = stream(pHole).distinct()
                ...
                // circular hole
                List<PolygonPoint> hole = IntStreams.range(0,num)
                ...
                // steiner points
                steiner = IntStreams.range(0,steinerPoints)
            ...
            if(pointsHoles!=null || rad>0d){
                Iterables.forEach(holes, hole->Iterables.forEach(hole, points1::add));
            } else {
                Iterables.forEach(steiner, points1::add);
            }

            int totalHolePoints=stream(holePoints).reduce(0, (a,b)->a+b);//Integer::sum
            int numPoints=extPoints+steinerPoints+totalHolePoints;

            FloatCollector pointsBottom = stream(points1)
                    .flatMapToDouble(p->DoubleStreams.of(p.getX(),p.getY(),0d))
                    .collect(()->new FloatCollector(points1.size()*3),FloatCollector::add,FloatCollector::join);
            FloatCollector pointsTop = stream(points1)
                    .flatMapToDouble(p->DoubleStreams.of(p.getX(),p.getY(),height.get()))
                    .collect(()->new FloatCollector(points1.size()*3),FloatCollector::add,FloatCollector::join);
            pointsBottom.join(pointsTop);
            points0=pointsBottom.toArray();
            numVertices=points0.length/3;

            FloatCollector texBottom = stream(points1)
                    .flatMapToDouble(p->DoubleStreams.of((p.getX()-minX)/(maxX-minX),(p.getY()-minY)/(maxY-minY)))
                    .collect(()->new FloatCollector(points1.size()*2),FloatCollector::add,FloatCollector::join);
            FloatCollector texTop = stream(points1)
                    .flatMapToDouble(p->DoubleStreams.of((p.getX()-minX)/(maxX-minX),(p.getY()-minY)/(maxY-minY)))
                    .collect(()->new FloatCollector(points1.size()*2),FloatCollector::add,FloatCollector::join);
            ...
            texCoord1 = IntStreams.range(0, numTexCoords)
                    ...
                    .collect(Collectors.toList());
            List<int[]> listIndices = stream(tri).map((DelaunayTriangle t)->{
                int[] pIndex=new int[3];
                for(int j=0; j<3; j++){
                    final TriangulationPoint dt = t.points[j];
                    int[] toArray = IntStreams.range(0,points1.size())
            ...
            // base
            IntStream streamBottom = stream(listIndices)
                    .map(i->IntStreams.of(i[0], i[0], i[2], i[2], i[1], i[1]))
                    .flatMapToInt(i->i);
            // top
            IntStream streamTop = stream(listIndices)
                    .map(i->IntStreams.of(numPoints+i[0], numPoints+i[0], numPoints+i[1], numPoints+i[1], numPoints+i[2], numPoints+i[2]))
                    .flatMapToInt(i->i);

            // vertical, exterior
            IntStream streamExtWalls = IntStreams.range(0, extPoints-1)
                    .mapToObj(i->IntStreams.of(i,i,i+1,i+1,i+1+numPoints,i+1+numPoints,
                                              i,i,i+1+numPoints,i+1+numPoints,i+numPoints,i+numPoints))
                    .flatMapToInt(i->i);
            // vertical, exterior, close polygon
            IntStream streamExtWallsClose = IntStreams.of(extPoints-1,extPoints-1,0,0,0+numPoints,0+numPoints,
            ...
            if(totalHolePoints>0){
                ...
                IntStream streamIntWalls=IntStreams.empty();
                for(List<PolygonPoint> hole:holes){
                    acuHolePoints1=acuHolePoints0+hole.size()-1;
                    IntStream streamIntWallsHole = IntStreams.range(acuHolePoints0, acuHolePoints1)
                            .mapToObj(i->IntStreams.of(i,i,i+1+numPoints,i+1+numPoints,i+1,i+1,
                                                      i,i,i+numPoints,i+numPoints,i+1+numPoints,i+1+numPoints))
                            .flatMapToInt(i->i);
                    streamIntWalls=IntStreams.concat(streamIntWalls,streamIntWallsHole);
                ...
                IntStream streamIntWallsClose=IntStreams.empty();
                for(List<PolygonPoint> hole:holes){
                    acuHolePoints1=acuHolePoints0+hole.size()-1;
                    IntStream streamIntWallsCloseHole = IntStreams.of(acuHolePoints1,acuHolePoints1,
                            ...
                            numPoints+acuHolePoints0,numPoints+acuHolePoints0);
                    streamIntWallsClose=IntStreams.concat(streamIntWallsClose,streamIntWallsCloseHole);
                    acuHolePoints0=acuHolePoints1+1;
                }
                faces0=IntStreams.concat(streamBottom,
                    IntStreams.concat(streamTop,IntStreams.concat(streamExtWalls,
                        IntStreams.concat(streamExtWallsClose,IntStreams.concat(streamIntWalls,streamIntWallsClose)
                                )))).toArray();
            } else {
                faces0=IntStreams.concat(streamBottom,
                    IntStreams.concat(streamTop,IntStreams.concat(streamExtWalls,streamExtWallsClose))).toArray();
            }
        ...
        List<Face3> textures1 = IntStreams.range(0, faces0.length/6)
    ...
    private int[] getSmoothingGroups(List<Point3D> points, List<Face3> faces){
        return stream(faces).mapToInt(f->{
